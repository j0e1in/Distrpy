...
def _add_slots(t, begin, end, max_slots):
    def addslots(t, begin, end):
        m = t.talk('cluster', 'addslots', *range(begin, end))
...
def start_cluster_on_multi(host_port_list, max_slots=SLOT_COUNT):
    ...
    try:
        ...
        slots_each = int(SLOT_COUNT / len(talkers))
        slots_residue = int(SLOT_COUNT - slots_each * len(talkers))
        first_node_slots = int(slots_residue + slots_each)
...
def _check_master_and_migrate_slots(nodes, myself):
    ...
    mig_slots_to_each = int(len(myself.assigned_slots) / len(other_masters))
...
def shutdown_cluster(host, port):
    with Talker(host, port) as t:
        ...
        nodes_info = [_f for _f in m.split('\n') if _f]
        if len(nodes_info) > 1:
